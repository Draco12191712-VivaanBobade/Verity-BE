...
world.afterEvents.playerSpawn.subscribe((event) => {
  ...
  player.sendMessage("§7Verity is currently §eStable§7.");
  player.sendMessage(
    "§7Bugs, changes, and unfinished features may still occur though.",
  );
  ...
  player.sendMessage(
    "§7Messages sent to Verity are logged to a Discord server",
  );
  ...
  player.sendMessage(
    "§cPlease do not send sensitive or private information to Verity.",
  );
  ...
  player.sendMessage(
    "§7Verity is officially available through §eCurseForge§7 and §eMCPEDL§7.",
  );
  player.sendMessage(
    "§cDo not download Verity from unofficial websites or reuploads.",
  );
  player.sendMessage(
    "Join the discord server for support: https://discord.gg/affh5Xdvcg",
  );
  ...
  player.sendMessage("§c§lCopyright © Utlover65 Development Team");
  player.sendMessage("§4§lDO NOT DISTRIBUTE");
});
